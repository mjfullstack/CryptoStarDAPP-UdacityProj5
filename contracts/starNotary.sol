...
// Add a name and a symbol for your starNotary tokens
    ...
    // string public name = "CryptoStarToken";
...
function lookUptokenIdToStarInfo (uint _tokenId) public view returns (string memory) {
    ...
    // function lookUptokenIdToStarInfo(uint256 _tokenId) public view {
    //     return string memory tokenIdToStarInfo[_tokenId];
    }
    //
...
function exchangeStars(uint256 _tokenId1, uint256 _tokenId2) public {
    ...
    // function exchangeStars( uint256 _tokenId_1, uint256 _tokenId_2) public {
    //     // MUST be one of the owners requesting this exchange...
    ...
    //     address private tempAddr_1 = ownerOf( _tokenId_1 ); // Exchange requires saving 1 item temporarily
    //     address private tempAddr_2 = ownerOf( _tokenId_2 ); // Save 2nd address for check/final require stmt
    //     _removeTokenFrom( ownerOf( _tokenId_1 ), _tokenId_1 );
    //     _addTokenTo( ownerOf( _tokenId_2 ), _tokenId_1 );
    //     _removeTokenFrom( ownerOf( _tokenId_2 ), _tokenId_2 );
    //     _addTokenTo( tempAddr_1, _tokenId_2 );
    //     require( ownerOf( _tokenId_2 ) == tempAddr_1 );
    //     require( ownerOf( _tokenId_1 ) == tempAddr_2 );
    //     // Return the each star's info and new owner address
    //     return [ ownerOf( _tokenId_1 ), lookUptokenIdToStarInfo( _tokenId_1 ), _tokenId_1 ),
    //              ownerOf( _tokenId_2 ), lookUptokenIdToStarInfo( _tokenId_2 ), _tokenId_2 ];
    }
    //
...
function transferStar(address _to1, uint256 _tokenId) public {
    ...
    //     require( starsForSale[_tokenId] > 0);        // The star must be available; Required ???
    //     require( ownerOf(_tokenId) == msg.sender);   // Sender must be owner
    //     _removeTokenFrom( ownerOf( _tokenId ), _tokenId );
    //     _addTokenTo( to , _tokenId_1 );
    //     starsForSale[_tokenId] = 0;                 // Set new owner; Is this the right function to use ???
    //     require (ownerOf(_tokenId) == to );         // If trasnfer not successful, revert via require.
    //     return [ ownerOf(_tokenId), lookUptokenIdToStarInfo(_tokenId) ];   // Return the star's info and new owner address
    }
    //
